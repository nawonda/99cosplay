 ...
 *
 * @category   AllPay
 * @package    AllPay_Credit6
 * @copyright  Copyright (c) 2010 AllPay (http://www.allpay.com.tw)
 */
class AllPay_Credit6_Block_Redirect extends Mage_Core_Block_Template {
    ...
    protected function AutoSubmit() {
        ...
        if ($oOrder) {
            try {
                ...
                $oPayment->Send['ReturnURL'] = Mage::getUrl('credit6/processing/response');
                $oPayment->Send['ClientBackURL'] = Mage::getUrl('');
                $oPayment->Send['OrderResultURL'] = Mage::getUrl('credit6/processing/result');
                ...
                array_push(
									$oPayment->Send['Items']
									, array(
										'Name' => Mage::helper('credit6')->__('Commodity Group')
