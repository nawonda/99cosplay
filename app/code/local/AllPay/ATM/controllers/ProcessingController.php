 ...
 * @author shawn.chang
 */
class AllPay_ATM_ProcessingController extends Mage_Core_Controller_Front_Action {
    ...
    public function redirectAction() {
        try {
            ...
            if (!$oOrder->getId()) {
                Mage::throwException(Mage::helper('atm')->__('Order No not found'));
            }
            if ($oOrder->getState() != Mage_Sales_Model_Order::STATE_NEW) {
                $oOrder->setState(
                        Mage_Sales_Model_Order::STATE_NEW, Mage_Sales_Model_Order::STATE_NEW, Mage::helper('atm')->__('Redirect to pay page')
                )->save();
    ...
    protected function _processSale($request) {
        ...
        try {
            ...
            if ((int) $dePrice != $request['TradeAmt']) {
                Mage::throwException(Mage::helper('atm')->__('Transaction amount doesn\'t match.'));
            }
            ...
            if ($request['RtnCode'] == '2' && $oOrder->getState() == Mage_Sales_Model_Order::STATE_NEW) {
                ...
                $szComment = sprintf(Mage::helper('atm')->__('ATM Payment Information'), $szPaymentType, $szTradeDate, $szBankCode, $szVirtualAccount, $szExpireDate);
