 ...
 *
 * @category   AllPay
 * @package    AllPay_Credit3
 * @copyright  Copyright (c) 2010 AllPay (http://www.allpay.com.tw)
 */
class AllPay_Credit3_Block_Redirect extends Mage_Core_Block_Template {
    ...
    protected function AutoSubmit() {
        ...
        if ($oOrder) {
            try {
                ...
                $oPayment->Send['ReturnURL'] = Mage::getUrl('credit3/processing/response');
                $oPayment->Send['ClientBackURL'] = Mage::getUrl('');
                $oPayment->Send['OrderResultURL'] = Mage::getUrl('credit3/processing/result');
                ...
                array_push(
									$oPayment->Send['Items']
									, array(
										'Name' => Mage::helper('credit3')->__('Commodity Group')
