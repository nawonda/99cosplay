 ...
 * @author shawn.chang
 */
class AllPay_Credit3_ProcessingController extends Mage_Core_Controller_Front_Action {
    ...
    public function redirectAction() {
        try {
            ...
            if (!$oOrder->getId()) {
                Mage::throwException(Mage::helper('credit3')->__('Order No not found'));
            }
            if ($oOrder->getState() != Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
                $oOrder->setState(
                        Mage_Sales_Model_Order::STATE_PENDING_PAYMENT, Mage_Sales_Model_Order::STATE_PENDING_PAYMENT, Mage::helper('credit3')->__('Redirect to pay page')
                )->save();
