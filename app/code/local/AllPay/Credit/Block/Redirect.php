 ...
 */
class AllPay_Credit_Block_Redirect extends Mage_Core_Block_Template {
    ...
    protected function AutoSubmit() {
        ...
        if ($oOrder) {
            try {
                ...
                $oPayment->Send['ReturnURL'] = Mage::getUrl('credit/processing/response');
                $oPayment->Send['ClientBackURL'] = Mage::getUrl('');
                $oPayment->Send['OrderResultURL'] = Mage::getUrl('credit/processing/result');
                ...
                array_push($oPayment->Send['Items'], array('Name' => Mage::helper('credit')->__('Commodity Group'), 'Price' => (int) $oOrder->getGrandTotal(), 'Currency' => Mage::app()->getLocale()->currency(Mage::app()->getStore()->getCurrentCurrencyCode())->getSymbol(), 'Quantity' => 1, 'URL' => ''));
