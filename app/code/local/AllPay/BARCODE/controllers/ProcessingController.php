 ...
 * @author shawn.chang
 */
class AllPay_BARCODE_ProcessingController extends Mage_Core_Controller_Front_Action {
    ...
    public function redirectAction() {
        try {
            ...
            if (!$oOrder->getId()) {
                Mage::throwException(Mage::helper('barcode')->__('Order No not found'));
            }
            if ($oOrder->getState() != Mage_Sales_Model_Order::STATE_NEW) {
                $oOrder->setState(
                        Mage_Sales_Model_Order::STATE_NEW, Mage_Sales_Model_Order::STATE_NEW, Mage::helper('barcode')->__('Redirect to pay page')
                )->save();
    ...
    protected function _processSale($request) {
        ...
        try {
            ...
            if ($request['RtnCode'] == '10100073' && $oOrder->getState() == Mage_Sales_Model_Order::STATE_NEW) {
                ...
                $szComment = sprintf(Mage::helper('barcode')->__('BARCODE Payment Information'), $szPaymentType, $szTradeDate, $szBankCode, $szExpireDate, $szBarcode1, $szBarcode2, $szBarcode3);
