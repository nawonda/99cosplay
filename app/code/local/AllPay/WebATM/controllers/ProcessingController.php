 ...
 * @author shawn.chang
 */
class AllPay_WebATM_ProcessingController extends Mage_Core_Controller_Front_Action {
    ...
    public function redirectAction() {
        try {
            ...
            if (!$oOrder->getId()) {
                Mage::throwException(Mage::helper('webatm')->__('Order No not found'));
            }
            if ($oOrder->getState() != Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
                $oOrder->setState(
                        Mage_Sales_Model_Order::STATE_PENDING_PAYMENT, Mage_Sales_Model_Order::STATE_PENDING_PAYMENT, Mage::helper('webatm')->__('Redirect to pay page')
                )->save();
    ...
    protected function _processSale($request) {
        ...
        try {
            ...
            $szOrderID = ($this->_getConfigData('test_mode') ? str_replace($this->_getConfigData('test_order_prefix'), '', $request['MerchantTradeNo']) : $request['MerchantTradeNo']);
            $szReturnCode = $request['RtnCode'];
            $szPaymenttype = $request['PaymentType'];
            
            $oOrder = $this->_getOrder($szOrderID);
