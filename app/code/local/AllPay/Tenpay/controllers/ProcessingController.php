 ...
 * @author shawn.chang 
 */
class AllPay_Tenpay_ProcessingController extends Mage_Core_Controller_Front_Action {
    ...
    public function redirectAction() {
        try {
            ...
            if (!$oOrder->getId()) {
                Mage::throwException(Mage::helper('tenpay')->__('Order No not found'));
            }
            if ($oOrder->getState() != Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
                $oOrder->setState(
                        Mage_Sales_Model_Order::STATE_PENDING_PAYMENT, Mage_Sales_Model_Order::STATE_PENDING_PAYMENT, Mage::helper('tenpay')->__('Redirect to pay page')
                )->save();
